...
async function close(id, user) {
	...
	user = String(user);
	let {result} = await this.db.list.update(
		{_id: id, status: {$ne: 0}},
		{
...
async function open(id, {user, flag, category}) {
	...
	let {result} = await this.db.list.update({_id: id, status: 2}, {$set, $addToSet});
	if (result && result.ok && result.nModified) {
...
async function handle(id, {handle, group, flag, user, explain} = {}) {
	...
	const $set = {}, $addToSet = {historyUser:user};
	//状态
	...
	if (Array.isArray(newFlag)) {
		...
		$addToSet.historyFlag = {$each:$set.flag};
	}
	...
	//更新
	({result} = await this.db.list.update({_id: id, status: info.status, group: info.group, flag: info.flag}, {$set, $addToSet}));
	//日志
...
/** 获取用户可用标志 */
async function getUserFlag(group, ...args) {
	group = String(group);
	const groupInfo = this.group[group];
	if(!groupInfo) {
		return [];
	...
	try {
		const flags = await groupInfo.getUserFlag(group, ...args);
		if (Array.isArray(flags)) {
			return flags.map(x => x ? String(x) : null).filter(x=>x);
		}
		return [];
	} catch(e) {
		return [];
 ...
 * 设置工单组
 */
function setGroup(group, {getOption, handle, flag, title, option, getUserFlag}) {
	let groups = this.group;
	group = String(group);
	if (!group) {
		return false;
	...
	if (typeof getUserFlag !== "function") {
		return false;
	}
	const flags = {};
...
function getGroupTitleList() {
	...
	return Object.keys(group).map(k => [k, group[k].title]);
}
...
function getGroup(group) {
	...
	const {getOption, handle, getUserFlag, flag, title, option} = groups[group];
	const flags = {};
	for (let k in flag) {
		flags[k] = flag[k];
	}
	...
	return {title, getOption, handle, getUserFlag, flag: flags, option: options};
}


module.exports = async function init({collection}, init) {
	const ret = {};
	...
	if (init) {
		try {await that.db.navigation.ensureIndex({group: 1}, {name:"group"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({flag: 1}, {name:"flag"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({refid: 1}, {name:"refid"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({status: 1}, {name:"status"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({user: 1}, {name:"user"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({createDate: 1}, {name:"createDate"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({historyUser: 1}, {name:"historyUser"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({historyFlag: 1}, {name:"historyFlag"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({user: 1, group: 1}, {name:"user_group"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({group: 1, user: 1}, {name:"group_user"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({group: 1, flag: 1}, {name:"group_flag"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({flag: 1, group: 1}, {name:"flag_group"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({group: 1, refid: 1}, {name:"group_refid"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({refid: 1, group: 1}, {name:"refid_group"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({group: 1, historyFlag: 1}, {name:"group_historyFlag"})} catch(e) {}
		try {await that.db.navigation.ensureIndex({historyFlag: 1, group: 1}, {name:"historyFlag_group"})} catch(e) {}
		try {await that.db.log.ensureIndex({refid: 1}, {name:"refid"})} catch(e) {}
	}
	that.group = {};
	...
	ret.getGroupTitleList = getGroupTitleList.bind(that);
